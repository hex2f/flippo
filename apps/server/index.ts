...
import { SquareColor, type CardType } from './types';
...
export class Player {
	...
	constructor(public ws: ServerWebSocket<WSState> | undefined, public id: string, public name: string) {
		this.name = name;
		this.id = id;
	}
	...
	score() {
		...
		for (const player of this.players.values()) {
			...
			player.score = player.scoreStack.reduce((acc, card) => {
				...
				console.log('scoring', card.id, card.label, player.id, requestId)
